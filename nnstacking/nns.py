...
class AsymmetricLoss(nn.Module):
    def __init__(self):
        ...
        self.rev_sigmoid = lambda x, w, b: 1 + (1 / (1 + torch.exp(x * w + b)))

    def forward(self, x, y):
        bias = x - y
        w = self.rev_sigmoid(bias, 0.3, 5).reshape(x.shape[0], -1)
        mse = self.mse(x, y).reshape(x.shape[0], -1)
        weighted_mse = torch.multiply(mse, w)
...
class NNS(BaseEstimator):
    ...
    def __init__(
        ...
        self.es = es
        self.es_validation_set = es_validation_set
        self.es_give_up_after_nepochs = es_give_up_after_nepochs
        self.es_splitter_random_state = es_splitter_random_state
        self.nepoch = nepoch
        self.batch_initial = batch_initial
        self.batch_step_multiplier = batch_step_multiplier
        self.batch_step_epoch_expon = batch_step_epoch_expon
        self.batch_max_size = batch_max_size
        self.optim_lr = optim_lr
        self.batch_test_size = batch_test_size
        self.gpu = gpu
    ...
    def fit(self, x_train, y_train, predictions=None):
        ...
        elif self.criterion == "asymmetric":
            self.criterion = AsymmetricLoss()
        else:
            raise ValueError("criterion must be 'mse' or 'asymmetric'")
    ...
    def improve_fit(self, x_train, y_train, nepoch):
        ...
        if self.es:
            # ShuffleSplit is replaced with regular unshuffled split
            n_train = int((1 - self.es_validation_set) * nnx_train.shape[0])
            index_train = range(n_train)
            index_val = range(n_train, nnx_train.shape[0])
        ...
        es_penal_tries = 0
        for _ in tqdm(range_epoch, desc="Epochs"):
            batch_size = int(
                min(
                    batch_max_size,
                    self.batch_initial
